...
import { ColorValue, hslToString, createProfessionalPalette } from '@/lib/colorUtils'
...
export function DesignStudio({ 
  ...
  // Get industry preset and apply it
  const industryPreset = getIndustryPreset(industry)
  const [styleType, setStyleType] = useState<'professional' | 'modern' | 'creative' | 'elegant' | 'playful'>(industryPreset.styleType)
  const [primaryColor, setPrimaryColor] = useState<ColorValue>(industryPreset.colors.primary)
  const [secondaryColor, setSecondaryColor] = useState<ColorValue>(industryPreset.colors.secondary)
  const [tertiaryColor, setTertiaryColor] = useState<ColorValue>(industryPreset.colors.tertiary)
  ...
  // When industry changes, update colors and style type
  const handleIndustryChange = (value: string) => {
    if (!value) return
    setIndustry(value)
    
    // Apply industry preset
    const preset = getIndustryPreset(value)
  ...
  const handleGenerate = () => {
    // Apply professional color adjustments to prevent overly bright colors
    const adjustedColors = colorMode === 'manual' 
      ? createProfessionalPalette(primaryColor, secondaryColor, tertiaryColor)
      : {
              ...
              <div className="p-4 bg-primary/10 border border-primary/20 rounded-lg text-sm">
                ...
                <div className="text-foreground space-y-1">
                  <div className="flex items-center gap-2">
                    <div className="w-4 h-4 rounded" style={{ backgroundColor: `hsl(${industryPreset.colors.primary.hue}, ${industryPreset.colors.primary.saturation}%, ${industryPreset.colors.primary.lightness}%)` }} />
                    <span className="text-xs">Color palette tailored for {industryPreset.name.toLowerCase()}</span>
                  </div>
                  <div>• Style: <span className="font-medium capitalize">{industryPreset.styleType}</span></div>
                  <div className="text-xs text-muted-foreground mt-2">
                    {industryPreset.characteristics.join(' • ')}
                  </div>
                </div>
              </div>
            </div>
            ...
            <Card className="p-8 shadow-md border-2">
              ...
              {colorMode === 'manual' && (
                <>
                  <div className="mb-4 p-3 bg-primary/10 border border-primary/20 rounded-lg text-sm">
                    ...
                    <span className="font-medium text-primary">Professional Color Protection:</span>
                    <span className="text-muted-foreground ml-1">
                      Colors are automatically adjusted to maintain professional appearance (avoiding overly bright or neon colors)
                    </span>
                  </div>
              ...
              {colorMode === 'ai' && (
                <div className="text-sm text-muted-foreground bg-muted/50 rounded-lg p-4">
                  ...
                  AI will intelligently choose a professional color palette that matches the website content and creates optimal visual harmony. Colors will maintain professional saturation levels and avoid overly bright hues.
                </div>
              )}
