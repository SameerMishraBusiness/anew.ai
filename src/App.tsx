...
import { getIndustryPreset } from '@/lib/industryPresets'
...
Return ONLY the JSON array, no explanations.`
  ...
  const handleGenerateRedesign = async (options: {
    ...
    const industryPreset = getIndustryPreset(industry)
      ...
      if (colorMode === 'ai') {
        colorInstructions = 'Use industry-optimized palette for ' + industry + '\n' +
          `Primary: hsl(${industryPreset.colors.primary.hue}, ${industryPreset.colors.primary.saturation}%, ${industryPreset.colors.primary.lightness}%)\n` +
          `Secondary: hsl(${industryPreset.colors.secondary.hue}, ${industryPreset.colors.secondary.saturation}%, ${industryPreset.colors.secondary.lightness}%)\n` +
          `Tertiary: hsl(${industryPreset.colors.tertiary.hue}, ${industryPreset.colors.tertiary.saturation}%, ${industryPreset.colors.tertiary.lightness}%)\n` +
          `Characteristics: ${industryPreset.characteristics.join(', ')}`
      } else if (colorMode === 'current') {
        colorInstructions = 'Preserve the original website\'s color scheme. Extract primary/secondary/accent and adjust overly bright colors to professional levels.'
      } else {
        colorInstructions = 'Use user-provided palette:\n' +
      ...
      const prompt = `You are an expert web designer creating stunning, modern websites. I need you to create a complete, self-contained SINGLE-PAGE HTML file for a redesigned website.
...
2. INDUSTRY: ${industry.toUpperCase()}
   - This website is for the ${industry} industry
   - Design characteristics: ${industryPreset.characteristics.join(', ')}
   - Typography: Use ${industryPreset.fonts.heading} for headings, ${industryPreset.fonts.body} for body text
3. STYLE TYPE: ${styleType.toUpperCase()}
   ${styleGuidelines[styleType]}
${colorInstructions}
5. Additional changes requested: ${additionalChanges || 'None'}
...
DESIGN INSTRUCTIONS - PREMIUM QUALITY ONLY:
- Create ONE complete HTML file with embedded CSS and JavaScript
- Keep ALL original content and information from the website
- Make the layout modern, clean, and user-friendly with professional polish
- Use the specified colors throughout the design (primary for headers/CTAs, secondary for accents, tertiary for highlights)
- Ensure responsive design that works perfectly on mobile and desktop
- Include smooth transitions, hover effects, and smooth scroll behavior
- Make navigation and content hierarchy crystal clear and intuitive
- Add a sticky/fixed header with navigation links

IMAGE & VISUAL REQUIREMENTS - MAKE IT BEAUTIFUL:
- Add high-quality placeholder images using Unsplash URLs in this format:
  * Hero/banner images: https://images.unsplash.com/photo-[relevant-id]?w=1920&h=1080&fit=crop
  * Profile/team images: https://images.unsplash.com/photo-[relevant-id]?w=400&h=400&fit=crop&crop=faces
  * Product/service images: https://images.unsplash.com/photo-[relevant-id]?w=800&h=600&fit=crop
  * Use REAL Unsplash photo IDs that match the content (e.g., for a restaurant use food photos, for tech use modern office/tech photos)
- Add engaging icons using modern icon libraries (Font Awesome, Lucide, or Heroicons via CDN)
- Include visual elements: gradients, shadows (box-shadow), rounded corners (border-radius), and modern spacing
- Use CSS Grid and Flexbox for sophisticated layouts
- Add background patterns or subtle textures where appropriate
- Include visual separators between sections (waves, angles, or gradient overlays)

TYPOGRAPHY & COLOR ENHANCEMENTS:
- Use Google Fonts (Inter, Poppins, Playfair Display, or Montserrat) for modern typography
- Implement proper font hierarchy: 
  * Headers: 2.5rem-4rem, bold
  * Subheaders: 1.5rem-2rem, semi-bold
  * Body: 1rem-1.125rem, regular
- Use color gradients for CTAs and hero sections
- Ensure WCAG AA contrast ratios (4.5:1 for text, 3:1 for large text)
- Add subtle color variations (lighter/darker shades) for depth

MODERN UI PATTERNS TO INCLUDE:
- Hero section with compelling headline, subheadline, and CTA button
- Feature cards with icons, titles, and descriptions in a grid layout
- Testimonials or social proof section with avatars and quotes
- Call-to-action sections with gradient backgrounds and prominent buttons
- Footer with multiple columns (links, contact info, social media)
- Smooth hover effects on all interactive elements
- Mobile-friendly hamburger menu that slides in from the side
- Loading animations or fade-in effects on scroll (intersection observer)

CSS BEST PRACTICES:
- Use CSS custom properties (variables) for colors and spacing
- Implement a consistent spacing scale (8px grid: 8px, 16px, 24px, 32px, etc.)
- Add smooth transitions (transition: all 0.3s ease)
- Use modern shadows (box-shadow: 0 4px 6px rgba(0,0,0,0.1))
- Implement hover states for all clickable elements (scale, color change, shadow)
- Use min-height: 100vh for full-screen sections where appropriate
- Add padding and margins generously for breathing room

MAKE IT PRODUCTION-READY:
- NO placeholder text like "Lorem ipsum" - use real content from the original website
- NO broken images - all image URLs must be valid Unsplash URLs
- Ensure all interactive elements work (buttons, links, forms)
- Test mobile responsiveness with @media queries
- Add meta tags for SEO (viewport, description, Open Graph)
- Include favicon and proper title
- DO NOT include any explanations, comments, or markdown - just return the raw HTML
...
6. Maintain all high-quality images, icons, and visual elements
7. Keep the professional polish and modern design patterns
8. DO NOT include any explanations, comments, or markdown - just return the raw HTML

MAINTAIN QUALITY STANDARDS:
- Keep all Unsplash image URLs working and relevant
- Preserve all icons and visual elements
- Maintain smooth animations and transitions
- Keep all Google Fonts and typography hierarchy
- Preserve gradients, shadows, and modern styling
